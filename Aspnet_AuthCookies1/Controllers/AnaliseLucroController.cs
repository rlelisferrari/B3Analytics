...
namespace Aspnet_AuthCookies1.Controllers
{
    ...
    public class AnaliseLucroController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> IndexAjax(DadosFront dados)
        {
            ...
            if (!string.IsNullOrEmpty(NomeAcao))
            {
                var cotacoes = await _b3ApiService.GetIntradayByDate(NomeAcao, StringToDateTime(dados.list), 1);
                var relatorioAtivo = _b3ApiService.AnaliseLucroPorAtivo(cotacoes, NomeAcao, ConvertStringToFloat(Desagio), dataInicio, dataFim, horaInicio.AddHours(3), horaFim.AddHours(3));
                if (cotacoes == null || relatorioAtivo == null || relatorioAtivo.cotacoesIntraDay == null || cotacoes.Count == 0)
                {
                    ViewBag.Error = $"API não retorna cotações do ativo: {NomeAcao} nesta data";
                    return PartialView("TabelaLucro");
                }
                return PartialView("TabelaLucro", relatorioAtivo.cotacoesIntraDay);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> LucroResumidoAjax(DadosFront dados)
        {
            var nomeAcao = dados.nomeAcao;
            var desagio = dados.desagio;
            var dataInicio = DateTime.Parse(dados.dataInicio);
        ...
        [HttpPost]
        public async Task<IActionResult> ConsolidacaoAtivosAjax(DadosFront dados)
        {
            var Desagio = dados.desagio;
            var dataInicio = DateTime.Parse(dados.dataInicio);
            var dataFim = DateTime.Parse(dados.dataFim);
            ...
            if (!string.IsNullOrEmpty(Desagio))
            {
                try
                {
                    ...
                    {
                        string ativo = item;
                        await GeraConsilidacaoComVolumeByDate(ativo, StringToDateTime(dados.list), dataInicio, dataFim, Desagio, horaInicio.AddHours(3), horaFim.AddHours(3));
                    }
                    ...
                    var consolidado = _consolidacaoAtivos.Where(it => it.Entradas > 0).OrderByDescending(it => it.EntradasLucro).ToList();
                    ViewBag.Tempo = timeTaken;
                    return PartialView("TabelaConsolidacao", consolidado);
                }
                catch (Exception)
                {
                    ViewBag.Error = "Error";
                    return PartialView("TabelaConsolidacao");
                    throw;
                }
            }
        ...
        [HttpPost]
        public async Task<IActionResult> ConsolidacaoAtivosTesteAjax(DadosFront dados)
        {
            var Desagio = dados.desagio;
            var dataInicio = DateTime.Parse(dados.dataInicio);
            var dataFim = DateTime.Parse(dados.dataFim);
        ...
        public void InicializaFiltros(string NomeAcao, string Desagio, DateTime dataInicio, DateTime dataFim, DateTime horaInicio, DateTime horaFim)
        {
            ...
            ViewBag.HoraFim = horaFim < new DateTime(1800, 1, 1) ? new DateTime(date.Year, date.Month, date.Day, 18, 0, 0) : horaFim;
            ViewBag.Error = "";
        }
        ...
        private async Task GeraConsilidacaoComVolumeByDate(string ativo, List<DateTime> datas, DateTime dataInicio, DateTime dataFim, string desagio, DateTime horaInicio, DateTime horaFim)
        {
            var cotacoes = await _b3ApiService.GetIntradayByDate(ativo, datas, 1);
            if (cotacoes == null || cotacoes.Count == 0)
                this._logger.LogInformation($"API não retorna ativo: {ativo}");
            else
            {
